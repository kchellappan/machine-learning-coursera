...
J = (-1/m) * ones(1,num_labels)*(y1.*log(a3) + (1-y1).*log(1-a3))*ones(m,1);

regCost = ones(1,hidden_layer_size)*(Theta1(:,2:end).^2)*ones(input_layer_size,1) + ones(1,num_labels)*(Theta2(:,2:end).^2)*ones(hidden_layer_size,1);

J = J + (lambda/2/m)*regCost;

%Computing gradients via back propogation (semi-vectorized)
Del1 = 0;
Del2 = 0;
for i=1:m
	a1 = [1; X(i,:)'];
